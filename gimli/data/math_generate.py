...
def generate_problem(seed: int = None) -> Tuple[str, int]:
    ...
    answer = (
        int(num1_unspaced) + int(num2_unspaced)
        if operation == "+"
        else int(num1_unspaced) * int(num2_unspaced)
    )
...
def generate_and_save_json_file(file_index: int, problems_per_file: int) -> None:
    problems_list = [
        create_json_entry(*generate_problem()) for _ in tqdm(range(problems_per_file))
    ]
...
def write_problems_to_json_files(processes: int):
    ...
    with ProcessPoolExecutor(max_workers=processes) as executor:
        process_func = partial(
            generate_and_save_json_file, problems_per_file=problems_per_file
        )
...
def create_training_dataset_json(processes: int):
    ...
    print(
        f"JSON dataset creation complete. The files are saved in the directory: {DIRECTORY}"
    )
